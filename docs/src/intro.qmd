...
function evaluation(model::JointEnergyModel, val_set::DataLoader)
    ...
    num = 0
    for (x, y) in val_set
        ℓ_clf += sum(JointEnergyModels.class_loss(model, x, y))
        ℓ_gen += sum(JointEnergyModels.gen_loss(model, x))
        ℓ += JointEnergyModels.loss(model, x, y)
...
sampler = UnconditionalSampler(𝒟x, input_size=(D,D), batch_size=batch_size)
...
neach = 10
for i in 1:10
    x = jem.sampler(jem.chain, jem.sampling_rule; niter=n_iter)
    plts_i = []
    for j in 1:size(x, 3)
        xj = x[:,:,j]
...
```{julia}
logs = train_model(jem, train_set, opt_state; num_epochs=num_epochs, val_set=val_set)
```
