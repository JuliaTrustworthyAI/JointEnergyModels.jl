...
function langevine_dynamics_step(
    jem::JointEnergyModel, x, t::Int; 
    a::Float32=1.0f0, b::Float32=1.0f0, γ::Float32=0.9f0
)
    # Calculate gradient wrt xold
    grad_energy = energy_gradient(jem, x)

    # ϵ = a*(b + t)^-γ
    # η = Float32.(ϵ.*randn(size(grad_energy)))
    # Δx = ϵ / 2 * grad_energy + η

    η = Float32.(jem.σ .* randn(size(grad_energy)))
    Δx = jem.α * grad_energy + η
...
function sample(jem::JointEnergyModel, x)
    ...
    for i in 1:jem.ld_steps
        xsample = langevine_dynamics_step(jem, xsample, i)
    end
    return xsample
end
```
...
function samples_generated(model::JointEnergyModel, dl::DataLoader, n::Int=16; img_size=n_digits * 5)
    x = reduce((x, y) -> cat(x,y[1],dims=ndims(x)), dl, init = [])
    x = sample(model, x[:,:,rand(1:size(x)[end], n)])
    num_x = Int(round(ceil(sqrt(n))))
    num_y = Int(round(floor(sqrt(n))))
